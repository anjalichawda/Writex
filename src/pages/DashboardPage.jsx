...
const DashboardPage = () => {
  ...
  useEffect(()=>{
    const fetchBooks = async ()=>{
      try {
        const response = await aixosInstance.get(API_PATHS.BOOKS.GET_BOOKS);
  ...
  const handleDeleteBook =async ()=>{
     ...
     try {
      await aixosInstance.delete(`${API_PATHS.BOOKS.DELETE_BOOK}/${bookToDelete}`);
  ...
  const handleeBookCreated = (bookId)=>{
      ...
      <div className='container mx-auto p-6'>
        ...
        {isLoading  ? (
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
            {Array.from({length:4}).map((_,i)=>(<BookCardSkeleton key={i}></BookCardSkeleton>))}
          </div>
        ): books.length === 0 ? (
          <div className='flex flex-col items-center justify-center py-12 text-center border-2 border-dashed border-slate-200 rounded-xl mt-8 '>
            <div className='w-16 h-16 bg-slate-100 rounded-full flex  items-center justify-center mb-4'>
              <Book className='w-8 h-8 text-slate-400'></Book>
            </div>
            <h3 className='text-lg font-medium text-slate-900 mb-2'>No eBooks Found</h3>
            <p className='text-slate-500 mb-6 max-w-md'>You haven't created any eBooks yet. Get started by creating your frst one.</p>
        ...
        <CreateBookModal isOpen={isCreateModal}
          onClose={()=>setIsCreateModal(false)}
          onBookCreated={handleeBookCreated}/>
      </div>
