...
const CreateBookModal = ({isOpen,onClose,onBookCreated}) => {
  ...
  const handleGeneratingOutline = async ()=>{
    try{
      const response = await aixosInstance.post(API_PATHS.AI.GENERATE_OUTLINE,{
        topic: bookTitle,
        description: aiTopic || "",
        style: aiStyle,
 ...
 setChapters(chapters.filter((_,i)=> i!==index));
  ...
  const handleFinalizeBook =async ()=>{
    ...
    try {
      const response = await aixosInstance.post(API_PATHS.BOOKS.CREATE_BOOKS,{
        title:bookTitle,
        author: user.name || "unknown Author",
        chapters:chapters
      });
      toast.success("eBook created successfully!");
      onBookCreated(response.data.id);
      onClose();
      resetModal();
    } catch(error){
      ...
      {step === 1 && (
        <div className='space-y-5'>
         ...
         max="20">
          ...
          <InputField icon={Lightbulb}
          label="Topic (optinoal)"
      ...
      {step === 2 && (
        <div className='space-y-5'>
          ...
          className='space-y-3 max-h-96 overflow-y-auto pr-1'>
            {chapters.length===0? (
            ...
            </div>):(Array.isArray(chapters) && chapters.map((chapter,index)=>(
              <div key={index} 
              className='group p-4 border border-gray-200 rounded-xl hover:border-gray-300 hover:shadow-sm transition-all bg-white'>
                <div className='flex items-start gap-3 mb-3'>
                  ...
                  onChange={(e)=>handleChapterChange(index,"title",e.target.value)} placeholder='Chapter Title'
                ...
                <textarea value={chapter.description}
          ...
          <div className='flex items-center justify-between pt-4 border-t border-gray-100'>
            ...
            <div className='flex gap-2'>
              ...
              onClick={handleAddChapter} icon={Plus}>
                Add Chapter
              </Button>
